...
export const useModelsData = () => {
  ...
  const loadVendors = async () => {
    try {
      const res = await API.get('vendors/?page_size=1000');
      if (res.data.success) {
  ...
  const loadModels = async (
    ...
    setLoading(true);
    try {
      let url = `models/?p=${page}&page_size=${size}`;
      if (vendorKey && vendorKey !== 'all') {
        // Filter by vendor ID
        url = `models/search?vendor=${vendorKey}&p=${page}&page_size=${size}`;
      }
  ...
  const syncUpstream = async (opts = {}) => {
    ...
    try {
      ...
      const res = await API.post('models/sync_upstream', body);
      const { success, message, data } = res.data || {};
      if (success) {
  ...
  const previewUpstreamDiff = async (opts = {}) => {
    ...
    setPreviewing(true);
    try {
      const url = `models/sync_upstream/preview${locale ? `?locale=${locale}` : ''}`;
      const res = await API.get(url);
      const { success, message, data } = res.data || {};
      if (success) {
  ...
  const searchModels = async () => {
    ...
    try {
      const res = await API.get(
        `models/search?keyword=${searchKeyword}&vendor=${searchVendor}&p=1&page_size=${pageSize}`,
      );
      const { success, message, data } = res.data;
      if (success) {
        const newPageData = extractItems(data);
  ...
  const manageModel = async (id, action, record) => {
    ...
    switch (action) {
      case 'delete':
        res = await API.delete(`models/${id}`);
        break;
      case 'enable':
        res = await API.put('models/?status_only=true', { id, status: 1 });
        break;
      case 'disable':
        res = await API.put('models/?status_only=true', { id, status: 0 });
        break;
      default:
        return;
    }
