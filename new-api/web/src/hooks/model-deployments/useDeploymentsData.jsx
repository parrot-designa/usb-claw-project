...
export const useDeploymentsData = () => {
  ...
  const fetchDeployments = async ({ page, size, keyword, status }) => {
    ...
    try {
      ...
      if (isSearchMode) {
        ...
        url = `deployments/search?${params.toString()}`;
      } else {
        url = `deployments/?p=${page}&page_size=${size}`;
      }
  ...
  const startDeployment = async (deploymentId) => {
    try {
      const res = await API.post(`deployments/${deploymentId}/start`);
      if (res.data.success) {
  ...
  const restartDeployment = async (deploymentId) => {
    try {
      const res = await API.post(`deployments/${deploymentId}/restart`);
      if (res.data.success) {
  ...
  const syncDeploymentToChannel = async (deployment) => {
    ...
    try {
      const containersResp = await API.get(
        `deployments/${deployment.id}/containers`,
      );
      ...
      const createResp = await API.post('channel/', payload);
  ...
  const updateDeploymentName = async (deploymentId, newName) => {
    try {
      const res = await API.put(`deployments/${deploymentId}/name`, {
        name: newName,
      });
      if (res.data.success) {
  ...
  const batchDeleteDeployments = async () => {
    ...
    try {
      ...
      const res = await API.post('deployments/batch_delete', { ids });
      if (res.data.success) {
