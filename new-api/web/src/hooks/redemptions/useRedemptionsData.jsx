...
export const useRedemptionsData = () => {
  ...
  const loadRedemptions = async (page = 1, pageSize) => {
    ...
    try {
      const res = await API.get(
        `redemption/?p=${page}&page_size=${pageSize}`,
      );
  ...
  const searchRedemptions = async () => {
    ...
    try {
      const res = await API.get(
        `redemption/search?keyword=${searchKeyword}&p=1&page_size=${pageSize}`,
      );
      const { success, message, data } = res.data;
      if (success) {
        const newPageData = data.items;
    ...
    try {
      switch (action) {
        ...
        case REDEMPTION_ACTIONS.ENABLE:
          ...
          res = await API.put('redemption/?status_only=true', data);
          break;
  ...
  const batchDeleteRedemptions = async () => {
    Modal.confirm({
      ...
      onOk: async () => {
        setLoading(true);
        const res = await API.delete('redemption/invalid');
        const { success, message, data } = res.data;
        if (success) {
