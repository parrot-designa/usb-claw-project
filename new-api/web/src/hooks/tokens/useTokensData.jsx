...
export const useTokensData = (openFluentNotification, openCCSwitchModal) => {
  ...
  const loadTokens = async (page = 1, size = pageSize) => {
    setLoading(true);
    setSearchMode(false);
    const res = await API.get(`token/?p=${page}&size=${size}`);
  ...
  const manageToken = async (id, action, record) => {
    ...
    switch (action) {
      ...
      case 'enable':
        data.status = 1;
        res = await API.put('token/?status_only=true', data);
        break;
      case 'disable':
        data.status = 2;
        res = await API.put('token/?status_only=true', data);
  ...
  const searchTokens = async (page = 1, size = pageSize) => {
    ...
    const res = await API.get(
      `token/search?keyword=${encodeURIComponent(searchKeyword)}&token=${encodeURIComponent(searchToken)}&p=${normalizedPage}&size=${normalizedSize}`,
    );
    const { success, message, data } = res.data;
    if (success) {
      setSearchMode(true);
  ...
  const batchDeleteTokens = async () => {
    ...
    try {
      ...
      const res = await API.post('token/batch', { ids });
      if (res?.data?.success) {
