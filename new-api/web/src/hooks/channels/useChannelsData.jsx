...
export const useChannelsData = () => {
  ...
  const loadChannels = async (
    ...
    const res = await API.get(
      `channel/?p=${page}&page_size=${pageSize}&id_sort=${idSort}&tag_mode=${enableTagMode}${typeParam}${statusParam}`,
    );
  ...
  const searchChannels = async (
    ...
    try {
      ...
      const statusParam = statusF !== 'all' ? `&status=${statusF}` : '';
      const res = await API.get(
        `channel/search?keyword=${searchKeyword}&group=${searchGroup}&model=${searchModel}&id_sort=${sortFlag}&tag_mode=${enableTagMode}&p=${page}&page_size=${pageSz}${typeParam}${statusParam}`,
      );
      const { success, message, data } = res.data;
      if (success) {
  ...
  const manageChannel = async (id, action, record, value) => {
    ...
    switch (action) {
      case 'delete':
        res = await API.delete(`channel/${id}/`);
        break;
      case 'enable':
        data.status = 1;
        res = await API.put('channel/', data);
        break;
      case 'disable':
        data.status = 2;
        res = await API.put('channel/', data);
  ...
  const manageTag = async (tag, action) => {
    ...
    switch (action) {
      case 'enable':
        res = await API.post('channel/tag/enabled', { tag: tag });
        break;
      case 'disable':
        res = await API.post('channel/tag/disabled', { tag: tag });
        break;
    }
    const { success, message } = res.data;
    if (success) {
  ...
  const copySelectedChannel = async (record) => {
    try {
      const res = await API.post(`channel/copy/${record.id}`);
      if (res?.data?.success) {
  ...
  const submitTagEdit = async (type, data) => {
    ...
    try {
      const res = await API.put('channel/tag', data);
      if (res?.data?.success) {
        showSuccess('更新成功！');
        await refresh();
      }
  ...
  const batchSetChannelTag = async () => {
    ...
    const res = await API.post('channel/batch/tag', {
      ids: ids,
  ...
  const batchDeleteChannels = async () => {
    ...
    const res = await API.post(`channel/batch`, { ids: ids });
    const { success, message, data } = res.data;
    if (success) {
  ...
  const testAllChannels = async () => {
    const res = await API.get(`channel/test`);
    const { success, message } = res.data;
    if (success) {
  ...
  const deleteAllDisabledChannels = async () => {
    const res = await API.delete(`channel/disabled`);
    const { success, message, data } = res.data;
    if (success) {
      showSuccess(
  ...
  const updateAllChannelsBalance = async () => {
    const res = await API.get(`channel/update_balance`);
    const { success, message } = res.data;
    if (success) {
  ...
  const updateChannelBalance = async (record) => {
    ...
    const res = await API.get(`channel/update_balance/${record.id}/`);
  ...
  const fixChannelsAbilities = async () => {
    const res = await API.post(`channel/fix`);
    const { success, message, data } = res.data;
    if (success) {
      showSuccess(
  ...
  const checkOllamaVersion = async (record) => {
    try {
      const res = await API.get(`channel/ollama/version/${record.id}`);
      const { success, message, data } = res.data;
  ...
  const testChannel = async (
    ...
    try {
      let url = `channel/test/${record.id}?model=${model}`;
      if (endpointType) {
