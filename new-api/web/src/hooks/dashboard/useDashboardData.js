...
export const useDashboardData = (userState, userDispatch, statusState) => {
  ...
  const loadQuotaData = useCallback(async () => {
    ...
    try {
      ...
      if (isAdminUser) {
        url = `data/?username=${username}&start_timestamp=${localStartTimestamp}&end_timestamp=${localEndTimestamp}&default_time=${dataExportDefaultTime}`;
      } else {
        url = `data/self/?start_timestamp=${localStartTimestamp}&end_timestamp=${localEndTimestamp}&default_time=${dataExportDefaultTime}`;
      }
  ...
  const loadUserQuotaData = useCallback(async () => {
    ...
    try {
      ...
      const url = `data/users?start_timestamp=${localStartTimestamp}&end_timestamp=${localEndTimestamp}`;
      const res = await API.get(url);
      const { success, message, data } = res.data;
      if (success) {
        return data || [];
