...
const EditModelModal = (props) => {
  ...
  const fetchVendors = async () => {
    try {
      const res = await API.get('vendors/?page_size=1000'); // 获取全部供应商
      if (res.data.success) {
  ...
  const fetchPrefillGroups = async () => {
    try {
      const [tagRes, endpointRes] = await Promise.all([
        API.get('prefill_group?type=tag'),
        API.get('prefill_group?type=endpoint'),
      ]);
  ...
  const loadModel = async () => {
    ...
    setLoading(true);
    try {
      const res = await API.get(`models/${props.editingModel.id}`);
  ...
  const submit = async (values) => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        const res = await API.put('models/', submitData);
        ...
        const res = await API.post('models/', submitData);
        const { success, message } = res.data;
        if (success) {
