...
const EditChannelModal = (props) => {
  ...
  const loadChannel = async () => {
    setLoading(true);
    let res = await API.get(`channel/${channelId}`);
    if (res === undefined) {
      return;
    }
  ...
  const fetchUpstreamModelList = async (name, options = {}) => {
    ...
    if (isEdit) {
      ...
      const res = await API.get('channel/fetch_models/' + channelId, {
  ...
  const fetchModels = async () => {
    try {
      let res = await API.get(`channel/models`);
  ...
  const fetchModelGroups = async () => {
    try {
      const res = await API.get('prefill_group?type=model');
      if (res?.data?.success) {
  ...
  const handleRefreshCodexCredential = async () => {
    ...
    try {
      const res = await API.post(
        `channel/${channelId}/codex/refresh`,
        {},
        { skipErrorHandler: true },
      );
      if (!res?.data?.success) {
                      ...
                      {inputs.type === 22 && (
                        <div>
                          <Form.Input
                            ...
                            label={t('私有部署地址')}
                            placeholder={t(
                              '请输入私有部署地址，格式为：https://fastgpt.runopenapi',
                            )}
                            onChange={(value) =>
                              handleInputChange('base_url', value)
                            }
