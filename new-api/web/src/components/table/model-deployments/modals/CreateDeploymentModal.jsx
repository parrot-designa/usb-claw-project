...
const CreateDeploymentModal = ({ visible, onCancel, onSuccess, t }) => {
  ...
  const loadHardwareTypes = async () => {
    try {
      ...
      const response = await API.get('deployments/hardware-types');
  ...
  const loadAvailableReplicas = async (hardwareId, gpuCount) => {
    ...
    try {
      const response = await API.get(
        `deployments/available-replicas?hardware_id=${hardwareId}&gpu_count=${gpuCount}`,
      );
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      const response = await API.post('deployments', requestData);
