...
const OAUTH_PRESETS = {
  ...
  nextcloud: {
    ...
    token_endpoint: '/apps/oauth2v1/token',
...
const CustomOAuthSetting = ({ serverAddress }) => {
  ...
  const fetchProviders = async () => {
    setLoading(true);
    try {
      const res = await API.get('custom-oauth-provider/');
      if (res.data.success) {
  ...
  const handleDelete = async (id) => {
    try {
      const res = await API.delete(`custom-oauth-provider/${id}`);
      if (res.data.success) {
        showSuccess(t('删除成功'));
        fetchProviders();
      } else {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editingProvider) {
        res = await API.put(
          `custom-oauth-provider/${editingProvider.id}`,
          payload
        );
      } else {
        res = await API.post('custom-oauth-provider/', payload);
      }
  ...
  const handleFetchFromDiscovery = async () => {
    ...
    try {
      const res = await API.post('custom-oauth-provider/discovery', {
