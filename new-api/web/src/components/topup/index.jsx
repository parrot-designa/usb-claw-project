...
const TopUp = () => {
  ...
  const topUp = async () => {
    ...
    try {
      const res = await API.post('user/topup', {
        key: redemptionCode,
      });
  ...
  const onlineTopUp = async () => {
    ...
    try {
      ...
      if (payWay === 'stripe') {
        // Stripe 支付请求
        res = await API.post('user/stripe/pay', {
          amount: parseInt(topUpCount),
          payment_method: 'stripe',
        });
      } else {
        // 普通支付请求
        res = await API.post('user/pay', {
          amount: parseInt(topUpCount),
  ...
  const onlineCreemTopUp = async () => {
    ...
    setConfirmLoading(true);
    try {
      const res = await API.post('user/creem/pay', {
  ...
  const waffoTopUp = async (payMethodIndex) => {
    try {
        ...
        const res = await API.post('user/waffo/pay', requestBody);
        if (res !== undefined) {
            const { message, data } = res.data;
  ...
  const getUserQuota = async () => {
    let res = await API.get(`user/self`);
    const { success, message, data } = res.data;
    if (success) {
  ...
  const getSubscriptionSelf = async () => {
    try {
      const res = await API.get('subscription/self');
      if (res.data?.success) {
  ...
  const updateBillingPreference = async (pref) => {
    ...
    try {
      const res = await API.put('subscription/self/preference', {
  ...
  const getTopupInfo = async () => {
    try {
      const res = await API.get('user/topup/info');
  ...
  const transfer = async () => {
    ...
    const res = await API.post(`user/aff_transfer`, {
  ...
  const getAmount = async (value) => {
    ...
    try {
      const res = await API.post('user/amount', {
  ...
  const getStripeAmount = async (value) => {
    ...
    setAmountLoading(true);
    try {
      const res = await API.post('user/stripe/amount', {
        amount: parseFloat(value),
      });
      if (res !== undefined) {
        const { message, data } = res.data;
