...
export async function fetchTokenKey(tokenId) {
  const response = await API.post(`token/${tokenId}/key`);
...
export async function fetchTokenKeysBatch(tokenIds) {
  const response = await API.post('token/batch/keys', { ids: tokenIds });
  const { success, data, message } = response.data || {};
...
export async function fetchTokenKeys() {
  try {
    const response = await API.get('token/?p=1&size=10');
