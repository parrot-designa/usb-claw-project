...
export default function SettingsPerformance(props) {
  ...
  async function clearDiskCache() {
    try {
      const res = await API.delete('performance/disk_cache');
      if (res.data.success) {
  ...
  async function resetStats() {
    try {
      const res = await API.post('performance/reset_stats');
      if (res.data.success) {
        showSuccess(t('统计已重置'));
        fetchStats();
      }
  ...
  async function forceGC() {
    try {
      const res = await API.post('performance/gc');
      if (res.data.success) {
  ...
  async function fetchLogInfo() {
    try {
      const res = await API.get('performance/logs');
      if (res.data.success) {
  ...
  async function cleanupLogFiles() {
    ...
    try {
      const res = await API.delete(
        `performance/logs?mode=${logCleanupMode}&value=${logCleanupValue}`,
      );
      if (res.data.success) {
