...
func SetApiRouter(router *gin.Engine) {
	...
	apiRouter.Use(middleware.CORS()) // 允许跨域
	...
	apiRouter.Use(middleware.GlobalAPIRateLimit())
	{
		...
		userRoute := apiRouter.Group("/user")
		{
			...
			// topup/info 需要登录认证，topup/info/post 通过 body 中的 session_cookie 自行认证
			userRoute.GET("/topup/info", controller.GetTopUpInfo)
			userRoute.POST("/topup/info/post", controller.GetTopUpInfoBySession)
			userRoute.POST("/topup/self/post", controller.GetUserTopUpsBySession)
			userRoute.POST("/topup/post", controller.TopUpBySession)
			// pay/post 通过 body 中的 session_cookie 自行认证
			userRoute.POST("/pay/post", controller.RequestEpayBySession)
		...
		usbKeyRoute := apiRouter.Group("/usb_key")
		usbKeyRoute.Use(middleware.AdminAuth())
		{
			usbKeyRoute.GET("/", controller.GetAllUSBKeys)
			usbKeyRoute.GET("/search", controller.SearchUSBKeys)
			usbKeyRoute.GET("/:id", controller.GetUSBKey)
			usbKeyRoute.POST("/", controller.CreateUSBKey)
			usbKeyRoute.PUT("/", controller.UpdateUSBKey)
			usbKeyRoute.DELETE("/:id", controller.DeleteUSBKey)
		}
		// activate 接口完全公开，不受 AdminAuth 影响
