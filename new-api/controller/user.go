...
// sessionCookieName is the custom session cookie name for manual set-cookie
const sessionCookieName = "uclaw_session"

var sessionCodecs = securecookie.CodecsFromPairs([]byte(common.SessionSecret))

func encodeSessionCookie(data map[string]any) (string, error) {
	encoded, err := securecookie.EncodeMulti(sessionCookieName, data, sessionCodecs...)
	if err != nil {
		return "", err
	}
	return encoded, nil
...
func decodeSessionCookie(cookieValue string) (map[string]any, error) {
	var data map[string]any
	err := securecookie.DecodeMulti(sessionCookieName, cookieValue, &data, sessionCodecs...)
	if err != nil {
		return nil, err
	}
	return data, nil
...
func setupLogin(user *model.User, c *gin.Context) {
	...
	// 同时设置标准的 gin-contrib/sessions session（兼容浏览器自动 cookie）
	session := sessions.Default(c)
	...
	// 返回 session cookie 值，让客户端可以手动 set-cookie
	c.JSON(http.StatusOK, gin.H{
		"message": "",
		"success": true,
		"data": map[string]any{
		...
		"session_cookie": sessionCookie, // 客户端可手动 set-cookie
	})
}
...
// GetSelfBySession 获取当前登录用户信息（通过 session_cookie 认证，供 U-Claw 使用）
func GetSelfBySession(c *gin.Context) {
	...
	// 获取用户的令牌列表
	tokens, _ := model.GetAllUserTokens(userId, 0, 100)
	...
	// 获取用户充值总金额（美元）
	totalTopupMoney, _ := model.GetUserTotalTopupMoney(userId)
	usedBalance := float64(user.UsedQuota) / common.QuotaPerUnit
	remainBalance := totalTopupMoney - usedBalance
	...
	responseData := map[string]interface{}{
		...
		"quota":              user.Quota,
		"used_quota":         user.UsedQuota,
		"remain_quota":       user.Quota - user.UsedQuota,
		"total_balance":      totalTopupMoney,
...
// TopUpBySession 充值（通过 session_cookie 认证，供 U-Claw 使用）
func TopUpBySession(c *gin.Context) {
	// 从 JSON body 或 form data 获取 session_cookie
	var body struct {
		SessionCookie string `json:"session_cookie"`
		Key           string `json:"key"`
